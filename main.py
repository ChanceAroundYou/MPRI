...
if __name__ == "__main__":
    ...
    parser.add_argument('-l', '--label', required=True, help='''The lable path LABLE of correspounding image
                            from AccuBrain.''')
    parser.add_argument('-o', '--output', help='''The output directory OUTPUT to save segmentation results and
                            other images. It must be given if the mode is seg or both and does
    ...
    mcp_mean_width, mcp_show_info = process.second_stage.run(
        image_nii, label_nii, quad_seg_point, mid_num, show_=show
    )
    scp_mean_width, scp_show_info = process.third_stage.run(
        image_nii, label_nii, rotate_quad_seg_point, mid_num, angle, show_=show
    )
    if args.mode == 'calc':
        process.output.calc(
